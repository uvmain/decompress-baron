...
async function createTarBuffer(
  entries: Array<
    | { name: string, content: string, linkTarget?: undefined }
    | { name: string, linkTarget: string, content?: undefined }
  >,
  gzip: boolean,
): Promise<Buffer> {
...
function createZipBuffer(
  ...
  for (const entry of entries) {
    ...
    const data = Buffer.from(
      isSymlink ? entry.linkTarget : (entry.content ?? ''),
      'utf-8',
    )
...
describe('decompress', () => {
  ...
  it('extract file', async () => {
    // tar.gz single file
    const tarBuf = await createTarBuffer(
      [{ name: 'file.txt', content: 'tar content' }],
      true,
    )
    const tarResult = await decompress(tarBuf)
    const tarFile = tarResult.find(
      f => f.type === 'file' && f.path === 'file.txt',
    )
    assert.ok(tarFile)
  ...
  it('extract tar.gz files', async () => {
    ...
    assert.strictEqual(
      files.find(f => f.path === 'dir/one.txt')?.data.toString(),
      'one',
    )
    assert.strictEqual(
      files.find(f => f.path === 'dir/two.txt')?.data.toString(),
      'two',
    )
    assert.strictEqual(
      files.find(f => f.path === 'root.txt')?.data.toString(),
      'root',
    )
    ...
    assert.ok(
      dirs.some(d => d.path === 'dir'),
      'should contain a directory entry for \'dir\'',
    )
  })
  ...
  it('extract multiple files', async () => {
    ...
    assert.strictEqual(
      zipFiles.find(f => f.path === 'x.txt')?.data.toString(),
      'xxx',
    )
    assert.strictEqual(
      zipFiles.find(f => f.path === 'y.txt')?.data.toString(),
      'yyy',
    )
  })
  ...
  it('accept a file path string as input', async () => {
    ...
    assert.strictEqual(
      files.find(f => f.path === 'alpha.txt')?.data.toString(),
      'alpha',
    )
    assert.strictEqual(
      files.find(f => f.path === 'beta.txt')?.data.toString(),
      'beta',
    )
